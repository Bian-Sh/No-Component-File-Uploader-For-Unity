...
Public Sub GetData (MaxSize)
	...
	if isGetData_=false then 
		...
		'取得每个项目之间的分隔符
		Dim bcrlfindex
		bcrlfindex=InStrB (1,RequestBinDate,bCrLf)
		'请注意：Unity post 出来的数据 bcrlfindex = 1，需要把这个跳过 
		if bcrlfindex = 1 then 
			'如果等于1 则说明首行需要跳过，重新算 换行符下标，暂不考虑出现多个换行符的情况,主要是填 Unity 的坑
			With oUpFileStream
				.Position = 3
				RequestBinDate =.Read
				.Position=0
				.Write RequestBinDate
				.Position=LenB(RequestBinDate)
				.SetEOS
			End With
			bcrlfindex=InStrB (1,RequestBinDate,bCrLf)
		end if
			sSpace = MidB (RequestBinDate,1, bcrlfindex-1)		
			iStart = LenB(sSpace)
			iFormStart = iStart+2
